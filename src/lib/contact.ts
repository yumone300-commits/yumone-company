export type ContactPayload={company:string;name:string;role:string;phone:string;email:string;brand:string;stores:string;service:string;concern:string;budget:string;timing:string;consent:boolean};
...
export async function submitContact(payload:ContactPayload):Promise<ContactResult>{
 ...
 return {mode:'mock',message:'모의 제출이 완료되었습니다. 입력하신 내용은 저장·전송되지 않았으며 실제 상담 접수는 이루어지지 않았습니다.'};
}
