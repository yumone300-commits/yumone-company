...
export function ServiceExperience({href}:{href:string}) {
 ...
 return <div className={s.page}>
  ...
  <section className={s.section}><div className={`${s.container} ${s.introGrid}`}><Heading number="01 / 대상과 과제" title={child?'이런 업무를 준비하고 있다면':'이런 상황의 담당자에게 필요합니다.'} text={child?(profile?.audience||copy.audience):undefined}/><div>{child?<p className={s.context}>{profile?.strategy||detail?.description}</p>:<ul className={s.situations}>{copy.situations.map(x=><li key={x}>{x}</li>)}</ul>}<div className={s.questions}><h3>상담 전, 세 가지만 확인해 주세요.</h3><ol>{(child?[`현재 준비된 자료가 있나요? (${detail!.checks[0]})`,`${detail!.checks[1]}에서 우선 정리할 내용은 무엇인가요?`,`내부 담당자는 누구인가요? (${detail!.checks[2]})`]:copy.questions).map(q=><li key={q}>{q}</li>)}</ol></div></div></div></section>
